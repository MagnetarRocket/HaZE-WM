...
void SetShape( HAZEWMWindow *tmp_win, int w)
{
  XRectangle rect[7];
...
void SetUpFrame( HAZEWMWindow *t, int x, int y, int w, int h, Bool sendEvent )
{
	XWindowChanges xwcv;
	unsigned long xwcm;
 ...
 /* Horizontal Scroll width */
	...
	if( t->flags & SCROLL )
		...
		//t->attr.width-= 3;
		//t->attr.height-=3;		
		t->win_x = NORMAL_BORDERWIDTH;
		t->win_y = 0;
		t->attr.width-= NORMAL_BORDERWIDTH*2;
		t->attr.height-=NORMAL_BORDERWIDTH;		
		*/
		}
...
void DrawCloseBox( HAZEWMWindow *t, Bool on )
	{
	...
	if( on || Scr.flags&SYSTEM8 )
		{
		if( !XGetGCValues( dpy,
						  (Scr.flags&SYSTEM8?Scr.Gray4GC:Scr.DecorationsActiveBGGC),
						  mask, &xgcv ) )
			{
		...
		{
		if (!XGetGCValues( dpy,Scr.DecorationsInActiveBGGC,mask, &xgcv ))
			{
			fprintf( stderr, "Sorry Can not get GC Values 3\n");
			xgcv.foreground = WhitePixel( dpy, Scr.screen );
			}
	...
	if( on ) attributes_box.cursor = Scr.HAZEWMCursors[DESTROY];
	else attributes_box.cursor = Scr.HAZEWMCursors[DEFAULT];
	...
	if( on )
		...
		if( Scr.flags&SYSTEM8 )
			...
			auxgc=Scr.DecorationsActiveFGGC;
			//XFillRectangle( dpy, t->title_w, Scr.BlackGC,
	...
	else auxgc=Scr.DecorationsInActiveFGGC;
	
			XDrawLine( dpy, t->close_b, auxgc,
						2, 0, BOXSIZE-1, BOXSIZE-1-2 );
			XDrawLine( dpy, t->close_b, auxgc,
						2, 1, BOXSIZE-1-1, BOXSIZE-1-2 );
			XDrawLine( dpy, t->close_b, auxgc,
						1, 1, BOXSIZE-1-1, BOXSIZE-1-1 );
			XDrawLine( dpy, t->close_b, auxgc,
						1, 2, BOXSIZE-1-2, BOXSIZE-1-1 );
			XDrawLine( dpy, t->close_b, auxgc,
						0, 2, BOXSIZE-1-2, BOXSIZE-1 );

			XDrawLine( dpy, t->close_b, auxgc,
						BOXSIZE-1-2, 0, 0, BOXSIZE-1-2 );
			XDrawLine( dpy, t->close_b, auxgc,
						BOXSIZE-1-2, 1, 1, BOXSIZE-1-2 );
			XDrawLine( dpy, t->close_b, auxgc,
						BOXSIZE-1-1, 1, 1, BOXSIZE-1-1 );
			XDrawLine( dpy, t->close_b, auxgc,
						BOXSIZE-1-1, 2, 2, BOXSIZE-1-1 );
			XDrawLine( dpy, t->close_b, auxgc,
						BOXSIZE-1, 2, 2, BOXSIZE-1 );

		
	
}

void DrawMinMax( HAZEWMWindow *t, Bool on )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes_box;
	XGCValues xgcv;
	...
	if( on ) attributes_box.cursor = Scr.HAZEWMCursors[MINMAX_CURSOR];
	else attributes_box.cursor = Scr.HAZEWMCursors[DEFAULT];
	...
	if( on )
		...
		if( Scr.flags&SYSTEM8 )
			...
			XFillRectangle( dpy, t->title_w, Scr.BlackGC,
						   ...
						   BOXSIZE+4, BOXSIZE+1 );
			*/
			XFillRectangle( dpy, t->title_w, Scr.DecorationsActiveBGGC,
						   t->frame_w-BOXSIZE*2-3,(TITLE_HEIGHT-BOXSIZE)/2,
						   BOXSIZE+4, BOXSIZE+1 );
			}
		}
	else auxgc = Scr.DecorationsInActiveFGGC;
			...
			DrawShadowBox( 0, 0, BOXSIZE, BOXSIZE, t->minmax_b, 1,
						  auxgc, auxgc, SHADOW_ALL );
			DrawShadowBox( 1, 1, BOXSIZE-2, BOXSIZE-2, t->minmax_b, 1,
						  auxgc, auxgc, SHADOW_ALL );
			DrawShadowBox( 2, 2, BOXSIZE-4, BOXSIZE-4, t->minmax_b, 1,
						  auxgc, auxgc, SHADOW_ALL );

			DrawShadowBox( 0, BOXSIZE-7, 7, 7, t->minmax_b, 1,
						  auxgc, auxgc, SHADOW_ALL );
			DrawShadowBox( 1, BOXSIZE-6, 5, 5, t->minmax_b, 1,
						  auxgc, auxgc, SHADOW_ALL );
			DrawShadowBox( 2, BOXSIZE-5, 3, 3, t->minmax_b, 1,
						  auxgc, auxgc, SHADOW_ALL );
			/*			
			XDrawLine( dpy, t->minmax_b, auxgc,
						0, BOXSIZE-7, 0, BOXSIZE-1 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						1, BOXSIZE-7, 1, BOXSIZE-1 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						2, BOXSIZE-7, 2, BOXSIZE-1 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						0, BOXSIZE-1, 7, BOXSIZE-1 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						0, BOXSIZE-2, 7, BOXSIZE-2 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						0, BOXSIZE-3, 7, BOXSIZE-3 );
			...
			XDrawLine( dpy, t->minmax_b, auxgc,
						0, 2, BOXSIZE-1, 2 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						BOXSIZE-1, 0, BOXSIZE-1, BOXSIZE-1 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						BOXSIZE-2, 0, BOXSIZE-2, BOXSIZE-1 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						BOXSIZE-3, 0, BOXSIZE-3, BOXSIZE-1 );
			
			XDrawLine( dpy, t->minmax_b, auxgc,
						0, BOXSIZE-1, BOXSIZE-1, 0 );
			XDrawLine( dpy, t->minmax_b, auxgc,
						1, BOXSIZE-1, BOXSIZE-1, 1 );
			*/
}

void DrawShadeR( HAZEWMWindow *t, Bool on )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes_box;
	XGCValues xgcv;
	...
	if( on ){
		if( t->flags&SHADE )
			attributes_box.cursor = Scr.HAZEWMCursors[SHADER_DOWN_CURSOR];
		else
			attributes_box.cursor = Scr.HAZEWMCursors[SHADER_UP_CURSOR];
	}
	else attributes_box.cursor = Scr.HAZEWMCursors[DEFAULT];
	valuemask = CWBackPixel | CWCursor;
...
void SetTitleBar( HAZEWMWindow *t, Bool on_off )
{
	int w, offset;
	...
	if( on_off || Scr.flags&SYSTEM8 )
		{
		if( !XGetGCValues( dpy, Scr.TitleActiveBGGC, mask, &xgcv ) )
			{
			fprintf( stderr, "Sorry Can not get GC Values 4\n");
			xgcv.foreground = BlackPixel( dpy, Scr.screen );
			}
		attributes_title.background_pixel = xgcv.foreground;
		if( !XGetGCValues( dpy, Scr.TitleActiveFGGC, mask, &xgcv ) )
			{
			fprintf( stderr, "Sorry Can not get GC Values 3\n");
			xgcv.foreground = BlackPixel( dpy, Scr.screen );
			}
		...
		//attributes_box.background_pixel = WhitePixel( dpy, Scr.screen );		
		if( !XGetGCValues( dpy, Scr.TitleInActiveBGGC, mask, &xgcv ) )
			{
			fprintf( stderr, "Sorry Can not get GC Values 4\n");
			xgcv.foreground = WhitePixel( dpy, Scr.screen );
			}
	...
	//if( on_off ) attributes_title.cursor = Scr.HAZEWMCursors[TITLE_CURSOR];
	if( on_off ) attributes_title.cursor = Scr.HAZEWMCursors[DEFAULT];
	else     attributes_title.cursor = Scr.HAZEWMCursors[DEFAULT];
	valuemask = CWBackPixel | CWCursor;
	...
	if( on_off )
		  ...
		  //{
			for( lp=4; lp<14; lp+=3 )
				...
				if( Scr.d_depth>1 )
					...
					XDrawLine( dpy, t->title_w, Scr.TitleActiveBGGC,
					  		4, lp-1, t->frame_w-7, lp-1 );
				}	
			if( Scr.d_depth>1 )
				{
			...
			if( Scr.flags&SYSTEM8 )
				...
				DrawShadowBox( 0, 0, t->frame_w+1, TITLE_HEIGHT, t->title_w,
						  1, Scr.Gray2GC, Scr.TitleActiveBGGC, SHADOW_ALL );

			if( Scr.flags&SYSTEM8 )
				XFillRectangle( dpy, t->title_w, Scr.Gray4GC,
					   		(t->frame_w-w)/2-10, 1, w+20, TITLE_HEIGHT-2 );
			else
					XFillRectangle( dpy, t->title_w, Scr.TitleActiveBGGC,
					   		1, 1, t->frame_w+1, TITLE_HEIGHT-1 );
			
					   		
			//dispgc = Scr.WhiteGC;
			dispgc = Scr.TitleActiveFGGC;
			//}
		//		else
			//			{
		//xgcv.function = GXor;
		...
		//XSetFillStyle( dpy, Scr.BlackGC, FillSolid );
			//XFillRectangle( dpy, t->frame_w, Scr.BlackGC,
			//		   		1, 1, w-1, TITLE_HEIGHT-1 );						
		//XFillRectangle( dpy, t->title_w, Scr.BlackGC,
		//		   		1, 1, t->frame_w-1, TITLE_HEIGHT-2 );
			//DrawShadowBox( 0, 0, t->frame_w-2, TITLE_HEIGHT, t->title_w,
			//			  1, Scr.BlackGC, Scr.BlackGC, SHADOW_ALL );
			//	}
		}
	else
		{
...
void DrawSbarAnk( HAZEWMWindow *t, int context, Bool on_off )
{
	Window	win;
	int size, scale, lp;
	...
	if( on_off ){
		if( t->flags&SCROLL && size<0 ){
			...
			attributes.cursor = Scr.HAZEWMCursors[DEFAULT];
			valuemask = CWBackPixel | CWCursor;
...
void DrawSbarArrow( HAZEWMWindow *t, int context, Bool on_off )
{
	int size;
	unsigned int mask, valuemask;
	Window win;
	...
	case C_SBAR_UP:
		win = t->scroll_v[1];
		size = t->frame_h-t->win_h;
		cursor = Scr.HAZEWMCursors[SBARV_CURSOR];
		break;
	case C_SBAR_DOWN:
		win = t->scroll_v[2];
		size = t->frame_h-t->win_h;
	...
	case C_SBAR_LEFT:
		win = t->scroll_h[1];
		size = t->frame_w-t->win_w;
		cursor = Scr.HAZEWMCursors[SBARH_CURSOR];
		break;
	case C_SBAR_RIGHT:
		win = t->scroll_h[2];
		size = t->frame_w-t->win_w;
...
void DrawSbarBar( HAZEWMWindow *t, int context, Bool on_off )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes;
...
void DrawFrameShadow( HAZEWMWindow *t, Bool on )
{
	XSegment lines[4];
	int lp;
	...
	if( on ) attributes.cursor = Scr.HAZEWMCursors[MOVE];
	else attributes.cursor = Scr.HAZEWMCursors[DEFAULT];
	valuemask = CWCursor;
	...
	if( t->flags&( TITLE | SBARV | SBARH | RESIZER ))
		...
		DrawShadowBox( 0, 0, t->frame_w, t->frame_h, t->frame, 5,
					  Scr.BlackGC, Scr.BlackGC, SHADOW_ALL );
		DrawShadowBox( 3, 3, t->frame_w-6, t->frame_h-6, t->frame, 5,
					  Scr.BlackGC, Scr.BlackGC, SHADOW_ALL );
		/*if( Scr.flags&SYSTEM8 )
...
void DrawResizeBox( HAZEWMWindow *t, Bool on_off )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes;
	XGCValues xgcv;
	...
	if( on_off || Scr.flags&SYSTEM8)
		...
		if (Scr.flags&SYSTEM8)
			...
			{
			if( !XGetGCValues( dpy, Scr.DecorationsActiveBGGC, mask, &xgcv ) )
				{
				fprintf( stderr, "Sorry Can not get GC Values 4\n");
				xgcv.foreground = WhitePixel( dpy, Scr.screen );
				}
	...
	if( on_off ) attributes.cursor = Scr.HAZEWMCursors[RESIZE];	
	else     attributes.cursor = Scr.HAZEWMCursors[DEFAULT];
	...
	if( on_off )
		...
		if( Scr.flags&SYSTEM8 )
			...
			{			
			point = 6*SBAR_WH/19.+1;
			scale = 9*SBAR_WH/19.-0.5;
			
			DrawShadowBox( point, point,  scale,  scale, t->resize_b,
						  1, Scr.DecorationsActiveFGGC, Scr.DecorationsActiveFGGC, SHADOW_ALL );
			DrawShadowBox( point-1, point-1, scale+2, scale+2, t->resize_b,
						  1, Scr.DecorationsActiveFGGC, Scr.DecorationsActiveFGGC, SHADOW_ALL );
			DrawShadowBox( point-2, point-2, scale+4, scale+4, t->resize_b,
						  1, Scr.DecorationsActiveFGGC, Scr.DecorationsActiveFGGC, SHADOW_ALL );
			
			point = 3*SBAR_WH/19.+0.5;
			scale = 8*SBAR_WH/19.+0.5;
			XFillRectangle( dpy, t->resize_b, Scr.DecorationsActiveBGGC,
	...
	else	XUnmapWindow( dpy, t->resize_b );
...
void DrawAllDecorations( HAZEWMWindow *t, Bool on_off )
{
	/*
	if( t->flags&SBARV ){
