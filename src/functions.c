...
void IconZoom ( HAZEWMWindow *mw, Bool on )
{
...
void ShadeWindow( HAZEWMWindow *mw )
{
	...
	if( mw->flags&SHADER ){
		attr.cursor = Scr.HAZEWMCursors[SHADER_DOWN_CURSOR];
		valuemask = CWCursor;
...
void UnShadeWindow( HAZEWMWindow *mw )
{
	unsigned long valuemask;
	XSetWindowAttributes attr;
	...
	if( mw->flags&SHADER ){
		attr.cursor = Scr.HAZEWMCursors[SHADER_UP_CURSOR];
		valuemask = CWCursor;
		XChangeWindowAttributes( dpy, mw->shade_b, valuemask, &attr );
	}
...
HAZEWMWindow *ChoiseWindow( XEvent *ev, int cursor )
{
	Bool loop=True;
	...
	if(XFindContext(dpy, select_win, HAZEWMContext,	(caddr_t *)&tmp_win)==XCNOENT)
		tmp_win = NULL;
	return tmp_win;
}
...
void ChangeDesk( char *action )
{
	...
	ChangeMenuItemLabel( "DESK", str, str, NULL, SELECTON, M_COPY );
	sprintf( str, "Desk %d", newdesk );
	ChangeMenuItemLabel( "ICON", str, str, NULL, SELECTON|CHECKON, M_COPY );
	ChangeMenuItemLabel( "DESK", str, str, NULL, SELECTON|CHECKON, M_COPY );
	ChangeMenuLabel(&(Scr.DeskMenu),str,NULL);
...
void SelectWindow( char *action )
{
	...
	if( tmp->flags&HIDED ){
		for( testwin = (HAZEWMWindow *)Scr.HAZEWMRoot.next; testwin != NULL;
			testwin = (HAZEWMWindow *)testwin->next)
...
{
	HAZEWMWindow *tmp, *last, *start, *lastactive=NULL;
	char desk[8];
	int newdesk, lastdesk=0;
	Bool samedesk=False;

	if( !Scr.HAZEWMRoot.next )	return;

	if( !strcmp( action, "NextSameDeskWindow" ))		samedesk = True;

	for( last = Scr.HAZEWMRoot.next; last && last->next!=NULL;
		last = last->next );

	if( Scr.ActiveWin==NULL || Scr.ActiveWin->next==NULL ){
		tmp = Scr.HAZEWMRoot.next;
		start = last;
	}
	else{
	...
	while( (tmp->flags&HIDED || tmp->flags&SKIPSELECT ||
		...
		tmp = tmp->next;
		if( tmp==NULL )		tmp = Scr.HAZEWMRoot.next;
	}
...
void SelectPreviousWindow( char *action )
{
	...
	if( Scr.ActiveWin==NULL || Scr.ActiveWin->prev==&Scr.HAZEWMRoot ){
		tmp = last;
		start = Scr.HAZEWMRoot.next;
	}
	else{
	...
	while( (tmp->flags&HIDED || tmp->flags&SKIPSELECT ||
		  ...
		  tmp!=start ){
		tmp = tmp->prev;
		if( tmp==&Scr.HAZEWMRoot )	tmp = last;
	}
	if( tmp==Scr.ActiveWin || (samedesk && Scr.currentdesk!=tmp->Desk) )
		return;
	if( Scr.currentdesk != tmp->Desk ){
...
void HideWindow( HAZEWMWindow *mw )
{
	char builtaction[24];
	char *winname;
...
{
	HAZEWMWindow *NextActive, *testwin;
...
{
	HAZEWMWindow *tmp, *ActiveWin=NULL;
	...
	do{
		...
		if( Scr.flags&STARTING && ev.type==MapNotify &&
		   XFindContext( dpy, ev.xany.window,
						HAZEWMContext, (caddr_t *)&tmp_win )!=XCNOENT)
...
void DebugWinList( char *action )
{
	...
	printf("Start\n");
	for( tmp = Scr.HAZEWMRoot.next; tmp!=NULL; tmp = tmp->next ){
