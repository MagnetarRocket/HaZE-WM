...
void BalloonSize( int *lh, int *lw, int *offset, int *w, int *h, HAZEWMWindow *mw )
{
	int lp;
	char contents[256];
...
void DrawWindowBalloon( int xoff, int yoff, int lh, int offset, int lw, HAZEWMWindow *mw )
{
	int lp;
	char contents[255];
	int xinc, yinc;
...
void BalloonHelp( void )
{
	...
	if( XFindContext ( dpy, JunkChild, HAZEWMContext, (caddr_t *)&tmp_win )
	   == XCNOENT ){
		return;
	}
