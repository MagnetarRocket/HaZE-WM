...
void SetDeskTopNum( char *line, FILE *fp )
{
	...
	for( lp=0; lp<Scr.n_desktop; lp++ ){
		...
		if( lp!=Scr.currentdesk )
			...
			AddMenuItem( &(Scr.DeskMenu), label, action,
						NULL, NULL, NULL, SELECTON );
			}
		else
			{
	...
	if( Scr.LastActive!=NULL )
		free( Scr.LastActive );
	Scr.LastActive = calloc( Scr.n_desktop, sizeof(HAZEWMWindow));
}
...
void SetTitleActiveFG( char *line, FILE *fp )
	{
	char *color, *buf;

	buf = SkipSpace( SkipNonSpace(line) );
	color = strdup( buf );
	color[strlen(color)-1] = '\0';

	if( !strncmp( line, "TITLE_ACTIVE_FG", strlen("TITLE_ACTIVE_FG") ) )
		XSetForeground(dpy,Scr.TitleActiveFGGC,GetColor(color));
		
	free(buf);
	free(color);
	}
void SetTitleActiveBG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "TITLE_ACTIVE_BG", strlen("TITLE_ACTIVE_BG") ) )
		XSetForeground(dpy,Scr.TitleActiveBGGC,GetColor(color));
		
	free(buf);
	free(color);
	}
void SetTitleInActiveFG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "TITLE_INACTIVE_FG", strlen("TITLE_INACTIVE_FG") ) )
		XSetForeground(dpy,Scr.TitleInActiveFGGC,GetColor(color));
		
	free(buf);
	free(color);
	}
void SetTitleInActiveBG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "TITLE_INACTIVE_BG", strlen("TITLE_INACTIVE_BG") ) )
		XSetForeground(dpy,Scr.TitleInActiveBGGC,GetColor(color));
...
void SetDecorationsActiveFG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "DECORATIONS_ACTIVE_FG", strlen("DECORATIONS_ACTIVE_FG") ) )
		XSetForeground(dpy,Scr.DecorationsActiveFGGC,GetColor(color));
		
	free(buf);
	free(color);
	}
void SetDecorationsActiveBG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "DECORATIONS_ACTIVE_BG", strlen("DECORATIONS_ACTIVE_BG") ) )
		XSetForeground(dpy,Scr.DecorationsActiveBGGC,GetColor(color));
		
	free(buf);
	free(color);
	}
void SetDecorationsInActiveFG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "DECORATIONS_INACTIVE_FG", strlen("DECORATIONS_INACTIVE_FG") ) )
		XSetForeground(dpy,Scr.DecorationsInActiveFGGC,GetColor(color));
		
	free(buf);
	free(color);
	}
void SetDecorationsInActiveBG( char *line, FILE *fp )
	{
	char *color, *buf;
	...
	if( !strncmp( line, "DECORATIONS_INACTIVE_BG", strlen("DECORATIONS_INACTIVE_BG") ) )
		XSetForeground(dpy,Scr.DecorationsInActiveBGGC,GetColor(color));
...
config_func main_config[]={
	...
	// Jes
	{ "TITLE_ACTIVE_FG", SetTitleActiveFG },
	{ "TITLE_ACTIVE_BG", SetTitleActiveBG },
	{ "TITLE_INACTIVE_FG", SetTitleInActiveFG },
	{ "TITLE_INACTIVE_BG", SetTitleInActiveBG },
	{ "DECORATIONS_ACTIVE_FG", SetDecorationsActiveFG },
	{ "DECORATIONS_ACTIVE_BG", SetDecorationsActiveBG },
	{ "DECORATIONS_INACTIVE_FG", SetDecorationsInActiveFG },
	{ "DECORATIONS_INACTIVE_BG", SetDecorationsInActiveBG },
	{ NULL, 0 }
};
...
void ReadConfigFile( char *configfile )
{
	...
	if( !file ){
		if( strcmp( configfile, CONFIGNAME) ){
			sysrc = calloc( strlen(HAZEWMLIBDIR)+strlen(configfile)+2, 1 );
			sprintf( sysrc, "%s/%s", HAZEWMLIBDIR, configfile );
		}
		else{
			sysrc = calloc( strlen(HAZEWMLIBDIR)+strlen(configfile)+9, 1 );
			sprintf( sysrc, "%s/system%s", HAZEWMLIBDIR, configfile );
		}
