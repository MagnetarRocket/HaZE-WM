...
{
	Scr.HAZEWMCursors[DEFAULT] = XCreateFontCursor( dpy, XC_left_ptr );
	Scr.HAZEWMCursors[SYS] = XCreateFontCursor( dpy, XC_X_cursor );
	Scr.HAZEWMCursors[TITLE_CURSOR] = XCreateFontCursor( dpy, XC_hand2 );
	Scr.HAZEWMCursors[RESIZE] =
		XCreateFontCursor( dpy, XC_bottom_right_corner );
	Scr.HAZEWMCursors[MOVE] = XCreateFontCursor( dpy, XC_fleur );
	Scr.HAZEWMCursors[MENU] = XCreateFontCursor( dpy, XC_sb_left_arrow );
	Scr.HAZEWMCursors[WAIT] = XCreateFontCursor( dpy, XC_watch );
	Scr.HAZEWMCursors[SELECT] = XCreateFontCursor( dpy, XC_dot );
	Scr.HAZEWMCursors[DESTROY] = XCreateFontCursor( dpy, XC_pirate );
	Scr.HAZEWMCursors[SBARH_CURSOR] =
		XCreateFontCursor( dpy, XC_sb_h_double_arrow );
	Scr.HAZEWMCursors[SBARV_CURSOR] =
		XCreateFontCursor( dpy, XC_sb_v_double_arrow );
	Scr.HAZEWMCursors[MINMAX_CURSOR] = XCreateFontCursor( dpy, XC_sizing );
	Scr.HAZEWMCursors[SHADER_UP_CURSOR] =
		XCreateFontCursor( dpy, XC_based_arrow_up );
	Scr.HAZEWMCursors[SHADER_DOWN_CURSOR] =
...
void InitScrParams( void )
{
	...
	Scr.HAZEWMRoot.w = Scr.Root;
	Scr.HAZEWMRoot.prev = NULL;
	Scr.HAZEWMRoot.next = NULL;
	Scr.LastActive = calloc( 1, sizeof(HAZEWMWindow));
	...
	Scr.DeskMenu.m_item = NULL;
	Scr.ActiveWin = NULL;
	Scr.root_pushes = 0;
	Scr.pushed_window = &Scr.HAZEWMRoot;
	Scr.style_list = NULL;
...
void InitGCs( void )
{
	...
	Scr.TitleActiveBGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	Scr.TitleInActiveFGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );

	Scr.DecorationsInActiveFGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	Scr.DecorationsActiveBGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	
	
	gcv.foreground = WhitePixel( dpy, Scr.screen );
	gcv.background = BlackPixel( dpy, Scr.screen );
	Scr.WhiteGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	Scr.TitleInActiveBGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	Scr.TitleActiveFGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );

	Scr.DecorationsActiveFGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	Scr.DecorationsInActiveBGGC = XCreateGC( dpy, Scr.Root, gcm, &gcv );
	...
	for( lp=0; nchildren > lp; lp++ ){
		...
		if( children[lp] && MappedNotOverride(children[lp]) ){
			...
			if( XFindContext( dpy, children[lp],
							 HAZEWMContext, (caddr_t *)&t)!=XCNOENT)
...
void Reborder( Bool restart )
{
	...
	XGrabServer (dpy);
	InstallWindowColormaps( &Scr.HAZEWMRoot );	/* force reinstall */
	for (tmp = (HAZEWMWindow *)Scr.HAZEWMRoot.next; tmp != NULL;
		 tmp = (HAZEWMWindow *)tmp->next){
...
void SaveDesktopState( void )
{
	...
	for (t = Scr.HAZEWMRoot.next; t != NULL; t = t->next){
...
void usage( void )
{
	//fprintf( stderr, "HAZEWM Ver %s\n\n", VERSION );
	fprintf( stderr, "HAZEWM [-d display] [-f config_file]");
	fprintf( stderr, " [-debug]\n" );
}

XErrorHandler CatchRedirectError(Display *err_dpy, XErrorEvent *event)
{
	fprintf( stderr, "HAZEWM : another WM may be running.\n" );
	exit(1);
}

XErrorHandler HAZEWMErrorHandler(Display *err_dpy, XErrorEvent *event)
{
	char err_msg[80];
	...
	fprintf( stderr, "HAZEWM : X Protocol error\n" );
...
Window CreateStartWindow( void )
{
	...
	attributes.cursor = Scr.HAZEWMCursors[WAIT];
...
int main( int argc, char *argv[] )
{
	...
	if( Scr.flags & DEBUGOUT )
		fprintf( stderr, "Welcome to HAZEWM World !\n" );
#ifdef USE_LOCALE
    ...
    XSetErrorHandler((XErrorHandler)HAZEWMErrorHandler);
