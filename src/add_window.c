...
void create_resizebox( HAZEWMWindow *tmp_win )
{
	...
	attributes.cursor = Scr.HAZEWMCursors[RESIZE];
	...
	XSaveContext( dpy, tmp_win->resize_b, HAZEWMContext, (caddr_t)tmp_win );
}

void create_scrollbar( HAZEWMWindow *tmp_win )
{
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	int title_height;
	...
	if( tmp_win->flags&SBARV ){
		attributes.cursor = Scr.HAZEWMCursors[SBARV_CURSOR];
		...
		for( lp=0; lp<4; lp++ )
			XSaveContext( dpy, tmp_win->scroll_v[lp], HAZEWMContext, (caddr_t)tmp_win );
	}
	if( tmp_win->flags&SBARH ){
		attributes.cursor = Scr.HAZEWMCursors[SBARH_CURSOR];
		...
		for( lp=0; lp<4; lp++ )
			XSaveContext(dpy,tmp_win->scroll_h[lp],HAZEWMContext,(caddr_t)tmp_win);
	}
}

void create_titlebar( HAZEWMWindow *tmp_win )
{
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	...
	attributes.event_mask = ButtonPressMask | ButtonReleaseMask | ExposureMask;
	attributes.background_pixel = BlackPixel( dpy, Scr.screen );
	attributes.cursor = Scr.HAZEWMCursors[TITLE_CURSOR];
	tmp_win->title_w = XCreateWindow( dpy, tmp_win->frame,
									 Scr.flags&SYSTEM8?2:0, 0,
									 tmp_win->attr.width+SBAR_WH+3,
									 TITLE_HEIGHT, 0,
									 CopyFromParent, InputOutput,
									 CopyFromParent,
									 valuemask, &attributes );
	...
	if( tmp_win->flags&CLOSER ){
		attributes.cursor = Scr.HAZEWMCursors[DESTROY];
		tmp_win->close_b = XCreateWindow( dpy, tmp_win->title_w,
										 ...
										 CopyFromParent,
										 valuemask, &attributes );
		XSaveContext( dpy, tmp_win->close_b, HAZEWMContext, (caddr_t)tmp_win );
	}
	if( tmp_win->flags&MINMAXR ){
		attributes.cursor = Scr.HAZEWMCursors[SELECT];
		tmp_win->minmax_b = XCreateWindow( dpy, tmp_win->title_w,
										  ...
										  CopyFromParent,
										  valuemask, &attributes );
		XSaveContext( dpy, tmp_win->minmax_b, HAZEWMContext, (caddr_t)tmp_win );
	}
	if( tmp_win->flags&SHADER ){
		...
		tmp_win->shade_b = XCreateWindow( dpy, tmp_win->title_w,
										 ...
										 CopyFromParent,
										 valuemask, &attributes );
		XSaveContext( dpy, tmp_win->shade_b, HAZEWMContext, (caddr_t)tmp_win );
	}
 	XMapSubwindows( dpy, tmp_win->title_w );
	XSaveContext( dpy, tmp_win->title_w, HAZEWMContext, (caddr_t)tmp_win );
}

void FetchWmProtocols (HAZEWMWindow *tmp)
{
...
void GrabKeys( HAZEWMWindow *tmp_win )
{
	ShortCut *tmp;
...
void GetWindowSizeHints( HAZEWMWindow *mw )
{
    long supplied=0;
...
void SmartPlacement( HAZEWMWindow *t, int width, int height, int *x, int *y )
{
	int temp_h,temp_w;
	int test_x = 0,test_y = 0;
	int loc_ok = False, tw,tx,ty,th;
	HAZEWMWindow *test_window;
	...
	while( test_y+temp_h<Scr.MyDisplayHeight && !loc_ok ){
		...
		while( test_x+temp_w<Scr.MyDisplayWidth && !loc_ok ){
			loc_ok = True;
			test_window = Scr.HAZEWMRoot.next;
...
HAZEWMWindow *AddWindow( Window win )
{
	HAZEWMWindow *tmp_win, *last;
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	int title_height;
	...
	tmp_win = calloc( 1, sizeof( HAZEWMWindow ) );
	...
	if( Scr.ActiveWin )
		...
		for ( last = (HAZEWMWindow *)&Scr.HAZEWMRoot;
			 last->next != NULL;
			 last = last->next);
	if( last->next ){
	...
	last->next = (struct HAZEWMWindow *)tmp_win;
	tmp_win->prev = last;
	if( tmp_win==(HAZEWMWindow *)0 )	return NULL;
	tmp_win->flags = 0;
	tmp_win->w = win;
	...
	if ( XGetCommand( dpy, tmp_win->w, &client_argv, &client_argc ) ) {
		XrmParseCommand( &db, table, 2, "HAZEWM", &client_argc, client_argv );
		status = XrmGetResource( db, "HAZEWM.desk", "HAZEWM.Desk",
								&str_type, &rm_value );
	...
	XSaveContext( dpy, tmp_win->frame, HAZEWMContext, (caddr_t)tmp_win );
	XSaveContext( dpy, tmp_win->Parent, HAZEWMContext, (caddr_t)tmp_win );
	XSaveContext( dpy, tmp_win->w, HAZEWMContext, (caddr_t)tmp_win );
