...
void RedrawMenu( MenuLabel *m, Bool onoroff )
{
	...
	if( Scr.flags&SYSTEM8 )
		...
		{
		fore_pixel = BlackPixel( dpy, Scr.screen );
		back_pixel = WhitePixel( dpy, Scr.screen );
		}
...
/* Mask String for Mono Display */
			if( !(m->flags&ACTIVE) && Scr.d_depth<2 )
				...
				gcm = GCFunction;
				XChangeGC( dpy, Scr.WhiteGC, gcm, &gcv );
				XSetFillStyle( dpy, Scr.WhiteGC, FillTiled );
				XFillRectangle( dpy, m->LabelWin, Scr.WhiteGC, 0, 0,
							   m->LabelWidth, MENUB_H);
				XSetFillStyle( dpy, Scr.WhiteGC, FillSolid );
				gcv.function = GXcopy;
				XChangeGC( dpy, Scr.WhiteGC, gcm, &gcv );
				}
			}
...
void RedrawMenuBar( void )
{
	...
	if( Scr.flags&SYSTEM8 && !(Scr.flags&STARTING) ){
		...
		XFillArc( dpy, Scr.MenuBar, Scr.BlackGC, 0, 0,
				 14, 14, 180*64, -(90*64) );
		XFillArc( dpy, Scr.MenuBar, Scr.BlackGC, Scr.MyDisplayWidth-15, 0,
				 14, 14, 0, 90*64 );
	}
	if( !(Scr.flags&SYSTEM8) )
...
void DrawStringMenuBar( char *str )
{
	...
	if( width!=0 ){
		XFillRectangle( dpy, Scr.MenuBar, Scr.BlackGC,
					   (Scr.MyDisplayWidth-width)/2, 0, width, MENUB_H-1 );
	}
	if( strlen( str )!=0 ){
		StrWidthHeight( MENUBARFONT, &width, NULL, &offset,
					   str, strlen(str));
		XDRAWSTRING( dpy, Scr.MenuBar, MENUBARFONT, Scr.WhiteGC,
...
MenuLabel *DrawMenuItem( MenuLabel *ml, int sel, Bool on )
{
	...
	if( on ){
		r_tmpGC = Scr.flags&SYSTEM8?Scr.MenuSelectBlueGC:Scr.WhiteGC;
		s_tmpGC = Scr.BlackGC;
	}
	else{
		r_tmpGC = Scr.flags&SYSTEM8?Scr.MenuBlueGC:Scr.BlackGC;
		s_tmpGC = Scr.WhiteGC;
	}
	...
	if( mi->xpm ){
		...
		if( mi->xpm->kind==PIXMAP ){
			...
			if( mi->mode&ICONGRAY  ){
				...
				XChangeGC( dpy, Scr.WhiteGC, gcm, &gcv );
				XSetFillStyle( dpy, Scr.WhiteGC, FillTiled );
				XFillRectangle( dpy, ml->PullWin, Scr.WhiteGC, 21, top_y,
...
/* Mask String for Mono Display */
		if( mi->mode&STRGRAY && Scr.d_depth<2 ){
			...
			XChangeGC( dpy, Scr.WhiteGC, gcm, &gcv );
			XSetFillStyle( dpy, Scr.WhiteGC, FillTiled );
			XFillRectangle( dpy, ml->PullWin, Scr.WhiteGC, 0, top_y,
	...
	if( mi->label && mi->label[0]=='\0' ){
		if( Scr.d_depth>1 ){
			...
			XChangeGC( dpy, Scr.WhiteGC, gcm, &gcv );
			XSetDashes( dpy, Scr.WhiteGC, 0, dots, 2 );
			XDrawLine( dpy, ml->PullWin, Scr.WhiteGC,
					  0, top_y+ml->ItemHeight/2,
					  ml->MenuWidth, top_y+ml->ItemHeight/2);
			gcm = GCLineStyle;
...
Bool SwallowMenu( MenuLabel *ml )
{
	...
	do{
		...
		if( ev.type==MapNotify &&
		   XFindContext(dpy, ev.xany.window, HAZEWMContext, (caddr_t *)&tmp_win)
		   !=XCNOENT)
...
void CreateMenuItems( void )
{
	...
	Scr.DeskMenu.flags = ACTIVE | STICKLABEL;
	Scr.DeskMenu.xpm = NULL;
	Scr.DeskMenu.name = strdup( "DESK" );
	Scr.DeskMenu.LabelStr = "Desk 0";
	CreateMenuLabel( &Scr.DeskMenu );
...
void MapMenuBar( HAZEWMWindow *win )
{
	int left, right;
	MenuLabel *tmpml;
	MenuLink *mlink;
	...
	right = Scr.MyDisplayWidth-Scr.IconMenu.LabelWidth-Scr.DeskMenu.LabelWidth-10;
	XMoveWindow( dpy, Scr.DeskMenu.LabelWin, right, 1 );
	Scr.DeskMenu.LabelX = right;
	...
	XMapWindow( dpy, Scr.DeskMenu.LabelWin );
...
void ChangeMenuItemLabel( char *name, char *oldl, char *newl, char *action, int mm, int sm )
{
	...
	else if( !strcmp( name, "DESK" ) )		tmplabel = &(Scr.DeskMenu);
	else{
