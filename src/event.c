...
HAZEWMWindow *ColormapWindow = &Scr.HAZEWMRoot;

void send_clientmessage( Window w, Atom a, Time timestamp)
{
	HAZEWMWindow *tmp_win;
	XClientMessageEvent ev;

	if( XFindContext ( dpy, w, HAZEWMContext, (caddr_t *)&tmp_win )
	   == XCNOENT )
		tmp_win = NULL;
...
void InstallWindowColormaps (HAZEWMWindow *tmp)
{
...
void InstallRootColormap( void )
{
    ...
    if (Scr.root_pushes == 0){
		...
		InstallWindowColormaps(&Scr.HAZEWMRoot);
		Scr.pushed_window = tmp;
	}
    Scr.root_pushes++;
    return;
...
Bool GrabEvent( int cursor )
{
	...
	while((i<1000)&&(val=XGrabPointer(dpy, Scr.Root, True, mask,
								  ...
								  Scr.HAZEWMCursors[cursor], CurrentTime)!=
					 GrabSuccess)){
		i++;
		sleep_a_little(1000);
    }
...
void RestoreWithdrawnLocation( HAZEWMWindow *tmp, Bool restart )
{
	XWindowChanges xwc;
	Window root, child;
	int rx, ry;
...
int GetContext( HAZEWMWindow *t, XEvent *e, Window *w )
{
	int Context;
...
HAZEWMWindow *NextActiveWin( HAZEWMWindow *t )
{
	int lp;
	Window parent, *children;
	unsigned nchildren;
	HAZEWMWindow *NextActive;

	XQueryTree( dpy, Scr.Root, &Scr.Root, &parent, &children, &nchildren );
	for( lp=nchildren-1; lp>-1; lp-- ){
		if( XFindContext( dpy, children[lp], HAZEWMContext,
						 (caddr_t *)&NextActive )
		   !=XCNOENT &&
		   NextActive->w!=t->w &&
...
void SetMapStateProp( HAZEWMWindow *tmp_win, int state)
{
...
int GetMapStateProp( HAZEWMWindow *tmp_win )
{
	Atom *protocols = NULL;
	Atom atype;
	int aformat;
...
void Destroy( HAZEWMWindow *t )
{
	...
	XDeleteContext( dpy, t->Parent, HAZEWMContext );
	XDeleteContext( dpy, t->frame, HAZEWMContext );
	XDeleteContext( dpy, t->w, HAZEWMContext );
	if( t->flags&CLOSER )
		XDeleteContext( dpy, t->close_b, HAZEWMContext );
	if( t->flags&MINMAXR )
		XDeleteContext( dpy, t->minmax_b, HAZEWMContext );
	if( t->flags&SHADER )
		XDeleteContext( dpy, t->shade_b, HAZEWMContext );
	if( t->flags&TITLE )
		XDeleteContext( dpy, t->title_w, HAZEWMContext );
	if( t->flags&SBARV )
		for( lp=0; lp<4; lp++ )
			XDeleteContext( dpy, t->scroll_v[lp], HAZEWMContext );
	if( t->flags&SBARH )
		for( lp=0; lp<4; lp++ )
			XDeleteContext( dpy, t->scroll_h[lp], HAZEWMContext );
	if( t->flags&RESIZER )
		XDeleteContext( dpy, t->resize_b, HAZEWMContext );
	if( Scr.n_desktop>1 )
	...
	if( t==Scr.ActiveWin ){
		...
		if( Scr.ActiveWin && 
			...
			RaiseHAZEWMWindow( Scr.ActiveWin );
	}
...
/*	w = ev->xany.window;*/
	...
	if( XFindContext( dpy, w, HAZEWMContext, (caddr_t *)&tmp_win )==XCNOENT )
		return;
...
void handle_configure_request( XEvent *ev )
{
	...
	if (XFindContext (dpy, xcr->window, HAZEWMContext, (caddr_t *) &tmp_win) ==
		XCNOENT)
		tmp_win = NULL;
	if( !tmp_win ){
		wc.x = xcr->x;
...
void MoveWindow( HAZEWMWindow *mw, XEvent *evp )
{
	Bool isEnd = False;
...
void DisplayPush( Window win )
{
	...
	if( Scr.flags&SYSTEM8 ){
		...
		XDrawSegments( dpy, win, Scr.Gray4GC, lines_a, 4 );
		XDrawSegments( dpy, win, Scr.WhiteGC, lines_b, 4 );
		XDrawRectangle( dpy, win, Scr.WhiteGC, 0, 0, BOXSIZE-1, BOXSIZE-1 );
	}
	XSync( dpy, 0 );
}

void CloseWindow( HAZEWMWindow *mw, XEvent *evp )
{
...
void DrawResizeFrame( int x, int y, int w, int h, HAZEWMWindow *t )
{
	XSegment lines[10];
	int nl=0;
...
void ConstrainSize( HAZEWMWindow *tmp_win, int *widthp, int *heightp )
{
...
void ResizeWindow( HAZEWMWindow *mw, XEvent *evp, Bool s_move )
{
	Bool isEnd = False;
...
void MinMaxWindow( HAZEWMWindow *mw, XEvent *evp )
{
	Bool isEnd = False, isIn = True;
	XEvent ev;
	...
	if( isIn ){
		...
		if( mw->flags & MAXIMAIZED )
			...
			if( mw->flags&SCROLL )
				...
				if( (mw->frame_w<mw->win_w || mw->frame_h<mw->win_h) )
					{
					mw->frame_w = mw->win_w;
					mw->frame_h = mw->win_h;
					}	
				...
				{
				//Jes
				mw->size_w = DisplayWidth(dpy,Scr.screen);
				mw->size_h = DisplayHeight(dpy,Scr.screen)-MENUB_H;
...
void ShadeBox( HAZEWMWindow *mw, XEvent *evp )
{
	Bool isEnd = False, isIn = True;
	XEvent ev;
...
void DoubleClickEvent( XEvent *ev )
{
	...
	if( XFindContext( dpy, ev->xany.window, HAZEWMContext, (caddr_t *)&Tmp_win )
	   != XCNOENT ){
...
***/
void PressSbarAnker( Bool isHbar, HAZEWMWindow *tmp_win )
{
...
/***
  isHbar  if scroll bar is Horizontal, then true
***/
void PressSbar( Bool isHbar, Window pressedwin, HAZEWMWindow *tmp_win )
{
...
void handle_button_press( XEvent *ev )
{
	...
	//if ( ev->xbutton.button == Button2 )	return;
	...
	if( XFindContext( dpy, ev->xany.window, HAZEWMContext, (caddr_t *)&Tmp_win )
	   != XCNOENT )
		...
		if( Tmp_win != Scr.ActiveWin  )
			{
			//Jes
			ev->xany.window=ev->xmotion.window=ev->xbutton.window = Tmp_win->w;
			XSendEvent(dpy, Tmp_win->w, True, ButtonMotionMask, ev );
			//Jes
...
{
	HAZEWMWindow *Tmp_win, *ActiveWin;
	MenuLabel *Tmp_menu;
	int context;
	Window win;
...
void HandleMapRequest( Window win )
{
	...
	if(XFindContext(dpy, win, HAZEWMContext,
